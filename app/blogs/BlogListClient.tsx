...
export function BlogListClient({ articles }: BlogListClientProps) {
  ...
  const blogGallerySchema = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: "Blogs – The Carcino Foundation",
    url: "https://carcinofoundation.org/blogs",
    about: {
      "@type": "NGO",
    ...
    hasPart: articles.map((article) => ({
      ...
      url: `https://carcinofoundation.org/blogs/${article.slug}`,
      publisher: {
        "@type": "NGO",
        name: "The Carcino Foundation",
      },
  ...
  return (
    <>
      <Script
        id="blog-gallery-schema"
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(blogGallerySchema),
        }}
      />
      <div
  ...
  text-white py-8 px-4 sm:px-10">
                Our <br className="sm:hidden" /> Blogs
              </h1>
              <p className="text-lg text-muted-foreground font-dmsans mb-8 leading-[120%] font-light max-w-[80%]">
                With extensive hard work and highly strenuous fact checking, our Writing Team has led us to offer you a selection of curated blogs.
              </p>
            </div>
          </motion.div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((article) => {
                ...
                return (
                  <Link
                    key={article.id}
                    href={article.slug ? `/blogs/${article.slug}` : `/blogs/${article.id}`}
