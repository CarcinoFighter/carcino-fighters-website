...
async function run() {
  const results = [];
  const base = 'https://cms.carcino.work';
  
  results.push(await testEndpoint(base, '/api/content/blogs'));
  results.push(await testEndpoint(base, '/api/blogs'));

  fs.writeFileSync(path.join(__dirname, 'test-results-cms.json'), JSON.stringify(results, null, 2));
  console.log('Results written to tmp/test-results-cms.json');
}
